/**
  ...
  *         Algorithm: TwoDistinctPoints
  */

package contests.codeforces.cf_535

import scala.io.StdIn._
import scala.annotation.tailrec

object codeOpsTwoDistinctPoints {
  def readLineToList = readLine.replaceAll("\\s+$", "").split(" ").map(_.trim.toInt).toList
...
object TwoDistinctPoints {
  def main(args: Array[String]): Unit = {
    import codeOpsTwoDistinctPoints._
    val n = readLine.toInt
    loop(n) {
      val List(l1, r1, l2, r2) = readLineToList
